...
@section('content')
    <x-ui.page-header title="Drivers" description="Manage Driver master data.">
        <x-slot name="actions">
            <x-ui.button variant="primary" :href="route('masters.drivers.create')">Add Driver</x-ui.button>
        </x-slot>
    </x-ui.page-header>
        ...
        <x-ui.table>
            <x-slot name="head">
                <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">License No</th>
            ...
            @forelse ($items as $item)
                <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900">{{ $item->license_no }}</td>
                            ...
                            <td class="px-6 py-4 text-right text-sm space-x-2">
                                <x-ui.button variant="ghost" size="sm" :href="route('masters.drivers.edit', $item)">Edit</x-ui.button>
                                <form method="POST" action="{{ route('masters.drivers.destroy', $item) }}" class="inline" onsubmit="return confirm('Delete this record?')">
                                    @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="{{ count($items) > 0 ? 99 : 1 }}" class="px-6 py-8"><x-ui.empty-state title="No records" description="Create your first Driver." /></td></tr>
            @endforelse
        </x-ui.table>
