...
@section('content')
    <x-ui.page-header title="Vehicles" description="Manage Vehicle master data.">
        <x-slot name="actions">
            <x-ui.button variant="primary" :href="route('masters.vehicles.create')">Add Vehicle</x-ui.button>
        </x-slot>
    </x-ui.page-header>
        ...
        <x-ui.table>
            <x-slot name="head">
                <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Registration No</th>
            ...
            @forelse ($items as $item)
                <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900">{{ $item->registration_no }}</td>
                            ...
                            <td class="px-6 py-4 text-right text-sm space-x-2">
                                <x-ui.button variant="ghost" size="sm" :href="route('masters.vehicles.edit', $item)">Edit</x-ui.button>
                                <form method="POST" action="{{ route('masters.vehicles.destroy', $item) }}" class="inline" onsubmit="return confirm('Delete this record?')">
                                    @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="{{ count($items) > 0 ? 99 : 1 }}" class="px-6 py-8"><x-ui.empty-state title="No records" description="Create your first Vehicle." /></td></tr>
            @endforelse
        </x-ui.table>
