...
@section('content')
    <x-ui.page-header title="UOMs" description="Manage UOM master data.">
        <x-slot name="actions">
            <x-ui.button variant="primary" :href="route('masters.uoms.create')">Add UOM</x-ui.button>
        </x-slot>
    </x-ui.page-header>
    ...
    <x-ui.card>
        ...
        <x-ui.table>
            ...
            @forelse ($items as $item)
                <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-right text-sm space-x-2">
                                <x-ui.button variant="ghost" size="sm" :href="route('masters.uoms.edit', $item)">Edit</x-ui.button>
                                <form method="POST" action="{{ route('masters.uoms.destroy', $item) }}" class="inline" onsubmit="return confirm('Delete this record?')">
                                    @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="{{ count($items) > 0 ? 99 : 1 }}" class="px-6 py-8"><x-ui.empty-state title="No records" description="Create your first UOM." /></td></tr>
            @endforelse
        </x-ui.table>
