...
<div id="dashboard-analytics"
    ...
    <x-ui.card class="mb-4">
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            <div>
                ...
                <select name="date_range" class="rounded-lg border-gray-300 text-sm" onchange="this.form.submit()">
                    @foreach(['today'=>'Today','yesterday'=>'Yesterday','week'=>'This Week','month'=>'This Month','quarter'=>'Quarter','year'=>'Year','custom'=>'Custom'] as $val=>$label)
                        <option value="{{ $val }}" @selected(($dateRange ?? 'month')===$val)>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            @if(($dateRange ?? '') === 'custom')
                <x-ui.input name="date_from" type="date" label="From" value="{{ $from ?? '' }}" />
                <x-ui.input name="date_to" type="date" label="To" value="{{ $to ?? '' }}" />
                <x-ui.button type="submit" variant="secondary">Apply</x-ui.button>
            @endif
            <div class="text-xs text-slate-500 pb-2">Showing {{ $from ?? '' }} → {{ $to ?? '' }}</div>
        </form>
        <div class="mt-3 flex flex-wrap gap-3 text-sm">
            <span class="rounded-full bg-rose-50 text-rose-700 px-3 py-1">Frozen parties: {{ $stats['frozen_parties'] ?? 0 }}</span>
            <span class="rounded-full bg-amber-50 text-amber-700 px-3 py-1">Bounce risk: {{ $stats['bounce_risk'] ?? 0 }}</span>
            <span class="rounded-full bg-slate-100 text-slate-700 px-3 py-1">Dead stock: {{ $stats['dead_stock'] ?? 0 }}</span>
            <span class="rounded-full bg-emerald-50 text-emerald-700 px-3 py-1">Margin stub: ₹{{ number_format($stats['margin_stub'] ?? 0, 0) }}</span>
        </div>
    </x-ui.card>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 p-6 sm:p-8 text-white shadow-xl">
        ...
        <div class="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @endcan
                @hasanyrole('super-admin|client-admin|finance|sales-manager')
                ...
                @endhasanyrole
                @hasanyrole('owner|super-admin|client-admin|sales-manager|finance')
    ...
    {{-- KPI cards --}}
    @php
        $salesGrowth = (float) ($stats['sales_growth'] ?? 0);
        $salesGrowthLabel = ($salesGrowth >= 0 ? '+' : '').$salesGrowth.'% vs last month';
        $salesGrowthType = $salesGrowth >= 0 ? 'positive' : 'negative';
    @endphp
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3 items-stretch">
      <div class="min-w-0 h-full">
        <x-ui.stat-card
            label="Today's Sales"
            value="₹{{ number_format($stats['today_sales'] ?? 0, 0) }}"
            change="{{ $salesGrowthLabel }}"
            change-type="{{ $salesGrowthType }}"
            accent="indigo"
        >
            <x-slot:icon>
        ...
        <x-ui.stat-card
            label="Month Revenue"
            value="₹{{ number_format($stats['month_sales'] ?? 0, 0) }}"
            change="{{ ($stats['total_invoices'] ?? 0).' invoices this month' }}"
            change-type="neutral"
            accent="violet"
        >
            <x-slot:icon>
        ...
        <x-ui.stat-card
            ...
            change="{{ ($stats['approved_orders'] ?? 0).' awaiting conversion' }}"
            change-type="warning"
            accent="amber"
        >
            <x-slot:icon>
        ...
        <x-ui.stat-card
            ...
            value="₹{{ number_format($stats['today_collections'] ?? 0, 0) }}"
        ...
        <x-ui.stat-card
            ...
            change="{{ ($stats['pending_deliveries'] ?? 0).' pending deliveries' }}"
            change-type="warning"
            accent="sky"
        >
            <x-slot:icon>
