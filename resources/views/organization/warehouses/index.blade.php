...
@section('content')
<x-ui.page-header title="Warehouses">
<x-slot name="actions">
...
<x-ui.button variant="primary" :href="route('organization.warehouses.create')">Add</x-ui.button>
</x-slot>
</x-ui.page-header>
<x-ui.card>
...
<td class="px-3 py-2">{{ $item->branch?->name }} · {{ $item->company?->name }}</td>
<td class="px-3 py-2">{{ $item->is_active ? 'Active' : 'Inactive' }}{{ $item->is_default ? ' · Default' : '' }}</td>

<td class="px-3 py-2 text-right whitespace-nowrap"><x-ui.button variant="secondary" size="sm" :href="route('organization.warehouses.edit', $item)">Edit</x-ui.button></td>
</tr>
@empty
