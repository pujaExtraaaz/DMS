...
<x-ui.select name="invoice_id" label="Invoice" required>@foreach($invoices as $inv)<option value="{{ $inv->id }}" @selected(optional($invoice)->id == $inv->id)>{{ $inv->invoice_no }} — {{ $inv->customer->name }} (₹{{ number_format($inv->grand_total - $inv->paid_amount, 2) }} due)</option>@endforeach</x-ui.select>
<x-ui.input name="amount" label="Amount" type="number" step="0.01" :value="optional($invoice) ? $invoice->grand_total - $invoice->paid_amount : ''" required />
