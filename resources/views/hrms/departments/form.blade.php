@extends('layouts.dms')
@section('title', $item->exists ? 'Edit Department' : 'Create Department')
@section('content')
<x-ui.page-header :title="$item->exists ? 'Edit Department' : 'Create Department'"><x-slot name="actions"><x-ui.button variant="secondary" :href="route('hrms.departments.index')">Back</x-ui.button></x-slot></x-ui.page-header>
<x-ui.card><form method="POST" action="{{ $item->exists ? route('hrms.departments.update', $item) : route('hrms.departments.store') }}" class="space-y-4 max-w-xl">@csrf @if($item->exists) @method('PUT') @endif
