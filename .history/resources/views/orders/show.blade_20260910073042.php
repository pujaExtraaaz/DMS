                ...
                @if(in_array($order->status, ['pending', 'draft']))
                    ...
                    >
                        ...
                        <input
                            ...
                            value="{{ old('credit_override_reason') }}"
                            maxlength="1000"
                            placeholder="Credit override reason (if blocked)"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                @foreach($order->items as $item)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-right">
                            {{ $item->quantity }}
                            @if((float) $item->reserved_qty > 0 || (float) $item->back_order_qty > 0)
                                <div class="text-xs text-gray-500">
                                    R: {{ $item->reserved_qty }} · D: {{ $item->delivered_qty }} · BO: {{ $item->back_order_qty }}
                                </div>
                            @endif
                        </td>
        ...
        <x-ui.card title="Summary">
            <dl class="space-y-3 text-sm">
                ...
                <div class="flex justify-between">
                    <dt class="text-gray-500">Fulfilment</dt>
                    <dd>
                        {{ ucfirst($order->fulfilment_mode ?? 'van') }}
                        @if($order->back_order)
                            <span class="ml-1 text-xs text-amber-600">Back-order</span>
                        @endif
                    </dd>
                </div>

                <div class="flex justify-between">
                    <dt class="text-gray-500">Credit Check</dt>
                    <dd>
                        {{ ucfirst(str_replace('_', ' ', $order->credit_check_status ?? 'pending')) }}
                    </dd>
                </div>
