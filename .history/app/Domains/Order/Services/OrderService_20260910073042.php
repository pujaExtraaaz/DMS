...
use App\Domains\Sales\Services\CreditCheckService;
...
class OrderService
{
    ...
    public function create(array $data, User $actor): Order
    {
        return DB::transaction(function () use ($data, $actor) {
            ...
            $this->regionBrandPolicyService->assertItemsAllowed(
                $customer,
                $items,
            ...
            $order = Order::create([
                ...
                'fulfilment_mode' => $data['fulfilment_mode'] ?? 'van',
                'back_order' => false,
    ...
    public function update(
        ...
        return DB::transaction(function () use ($order, $data, $actor) {
            ...
            $lockedOrder->update([
                'customer_id' => $customer->id,
                'warehouse_id' => $data['warehouse_id'] ?? $lockedOrder->warehouse_id,
                'updated_by_name' => $actorName,
                'order_date' => $data['order_date'],
                'due_date' => $data['due_date'] ?? null,
                'fulfilment_mode' => $data['fulfilment_mode'] ?? $lockedOrder->fulfilment_mode ?? 'van',
                'subtotal' => $pricing['subtotal'],
                'discount_amount' => $pricing['discount'],
                'tax_amount' => $pricing['tax'],
                'grand_total' => $pricing['total'],
    ...
    public function approve(
        ...
        return DB::transaction(function () use ($order, $actor, $options) {
            $lockedOrder = Order::query()
                ->with(['items.product', 'items.uom', 'customer'])
                ->whereKey($order->id)
                ->lockForUpdate()
                ->firstOrFail();
            ...
            $credit = $this->creditCheckService->assertForOrder(
                $lockedOrder,
                $options['credit_override_reason'] ?? null
            );
            ...
            if (($lockedOrder->fulfilment_mode ?? 'van') === 'warehouse') {
                $hasBackOrder = $this->reserveWarehouseStock($lockedOrder);
            }
            ...
            $lockedOrder->update([
                ...
                'blocked_reason' => $credit['status'] === 'overridden'
                    ? ($options['credit_override_reason'] ?? null)
                    : null,
            ...
            $this->auditLogService->record(
                ...
                [
                    'credit_check_status' => $credit['status'],
                    'fulfilment_mode' => $lockedOrder->fulfilment_mode,
                    'back_order' => $hasBackOrder,
                ]
            );
    ...
    public function cancel(
        ...
        return DB::transaction(function () use ($order, $actor) {
            ...
            $this->releaseReservations($lockedOrder);
    ...
    /**
     * Reserve available stock for warehouse fulfilment without inflating stock_levels.
     * Shortfall is stored as back_order_qty on the line.
     */
    protected function reserveWarehouseStock(Order $order): bool
    {
        $hasBackOrder = false;
        ...
        foreach ($order->items as $item) {
            ...
            $onHand = (float) ($stockLevel?->quantity ?? 0);

            $alreadyReserved = (float) StockReservation::query()
                ->where('product_id', $item->product_id)
                ->where('uom_id', $item->uom_id)
                ->where('status', 'active')
                ->sum('quantity');

            $available = max(0, $onHand - $alreadyReserved);
            $ordered = (float) $item->quantity;
            $reserveQty = min($ordered, $available);
            $backOrderQty = round($ordered - $reserveQty, 4);
    ...
    protected function releaseReservations(Order $order): void
    {
